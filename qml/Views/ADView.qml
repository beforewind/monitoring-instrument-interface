...
Item {
    ...
    ChartView {
        ...
        ValueAxis {
            id: axisX
            titleText: "Time(s)"
            min: adsource.axisXmin
            max: adsource.axisXmax
        }
        ...
        Component.onCompleted: {
            adsource.generateData(n, dialX.scaleValue() * 60);
            redraw();
        }
    ...
    Item {
        ...
        ColumnLayout {
            ...
            Extras.Dial {
                ...
                onValueChanged: {
                    adsource.updateColCount(dialX.scaleValue() * 60);
                }
        ...
        onTriggered: {
            for(var i=0; i<chartView.n; i++)
                if(chartView.channelChecked[i]){
                    adsource.update(i, chartView.sensorSeries[i]);
                }
            while(axisX.phase + dialX.scaleValue() * 60 < adsource.phase)
